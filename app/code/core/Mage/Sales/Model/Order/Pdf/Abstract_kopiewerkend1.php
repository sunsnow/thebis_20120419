...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->setFillColor(new Zend_Pdf_Color_Html("#071d25"));
        ...
        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Order nummer: # ').$order->getRealOrderId(), 35, 770, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Datum: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 35, 760, 'UTF-8');
    ...
    protected function _setFontRegular($object, $size = 5)
    {
    ...
    protected function _setFontBold($object, $size = 5)
    {
    ...
    protected function _setFontItalic($object, $size = 5)
    {
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
    {
        foreach ($draw as $itemsProp) {
            ...
            foreach ($lines as $line) {
                $maxHeight = 0;
                foreach ($line as $column) {
                    $fontSize = empty($column['font_size']) ? 5 : $column['font_size'];
